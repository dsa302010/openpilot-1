...
def out_of_space_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NormalPermanentAlert("储存空间不足", f"{full_perc}% 已使用")
...
def process_not_running_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NoEntryAlert(msg, alert_text_1="程式未执行")


def comm_issue_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  bs = [s for s in sm.data.keys() if not sm.all_checks([s, ])]
  msg = ', '.join(bs[:4])  # can't fit too many on one line
  return NoEntryAlert(msg, alert_text_1="程式通讯故障")
...
def overheat_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NormalPermanentAlert("系统运作过热", f"{temp:.0f} °C")


def low_memory_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  return NormalPermanentAlert("记忆体不足", f"{sm['deviceState'].memoryUsagePercent}% 已使用")
...
def modeld_lagging_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  return NormalPermanentAlert("驾驶模型延迟", f"{sm['modelV2'].frameDropPerc:.1f}% 帧数遗失")
...
def invalid_lkas_setting_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NormalPermanentAlert("LKAS 设定无效", text)
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.invalidLkasSetting: {
    ...
    ET.NO_ENTRY: NoEntryAlert("LKAS 设定无效"),
  },
  ...
  EventName.fanMalfunction: {
    ET.PERMANENT: NormalPermanentAlert("风扇故障", "可能是硬体问题"),
  },
  ...
  EventName.cameraMalfunction: {
    ...
    ET.NO_ENTRY: NoEntryAlert("相机故障: 重新启动装置"),
  },
  # Camera framerate too low
  EventName.cameraFrameRate: {
    ET.PERMANENT: NormalPermanentAlert("相机帧率过低", "重新启动装置"),
    ET.SOFT_DISABLE: soft_disable_alert("相机帧率过低"),
    ET.NO_ENTRY: NoEntryAlert("相机帧率过低: 重新启动装置"),
  },
  ...
  EventName.pedalPressed: {
    ET.USER_DISABLE: EngagementAlert(AudibleAlert.disengage),
    ET.NO_ENTRY: NoEntryAlert("驾驶踩下油门或煞车",
  ...
  EventName.outOfSpace: {
    ...
    ET.NO_ENTRY: NoEntryAlert("储存空间不足"),
  },
  ...
  EventName.sensorDataInvalid: {
    ...
    ET.NO_ENTRY: NoEntryAlert("感测器数据无效"),
    ET.SOFT_DISABLE: soft_disable_alert("感测器数据无效"),
  },
  ...
  EventName.commIssue: {
    ET.SOFT_DISABLE: soft_disable_alert("程式通讯故障"),
    ET.NO_ENTRY: comm_issue_alert,
  },
  EventName.commIssueAvgFreq: {
    ET.SOFT_DISABLE: soft_disable_alert("程式间通讯频率过低"),
    ET.NO_ENTRY: NoEntryAlert("程式间通讯频率过低"),
  },

  EventName.selfdrivedLagging: {
    ET.SOFT_DISABLE: soft_disable_alert("系统延迟"),
    ET.NO_ENTRY: NoEntryAlert("Selfdrive 进程延迟: 重新启动装置"),
  },

  # Thrown when manager detects a service exited unexpectedly while driving
  EventName.processNotRunning: {
    ET.NO_ENTRY: process_not_running_alert,
    ET.SOFT_DISABLE: soft_disable_alert("程式未执行"),
  },
  ...
  EventName.lowMemory: {
    ET.SOFT_DISABLE: soft_disable_alert("记忆体不足: 请重启设备"),
    ET.PERMANENT: low_memory_alert,
    ET.NO_ENTRY: NoEntryAlert("记忆体不足: 请重启设备"),
  },
  ...
  EventName.usbError: {
    ET.SOFT_DISABLE: soft_disable_alert("USB 错误: 重新启动装置"),
    ET.PERMANENT: NormalPermanentAlert("USB 错误: 重新启动装置"),
    ET.NO_ENTRY: NoEntryAlert("USB 错误: 重新启动装置"),
  },
  ...
  EventName.relayMalfunction: {
    ...
    ET.PERMANENT: NormalPermanentAlert("线束继电器故障", "请检查硬体"),
  ...
  EventName.userBookmark: {
    ET.PERMANENT: NormalPermanentAlert("书签已储存", duration=1.5),
  },
