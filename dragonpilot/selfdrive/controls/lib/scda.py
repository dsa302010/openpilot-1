#!/usr/bin/env python3
"""
Speed Camera Detection & Adjustment (SCDA) - Optimized Edition (Aggressive Fix)
主要改進:
1. 放寬安全限制: 允許更晚、更急的煞車介入
2. 修復近距離直接放棄的問題 (Time Buffer)
3. 強制開啟 Debug Log
"""
import os
import csv
...
# --- [修正] 安全參數 (放寬限制) ---
MAX_COMFORTABLE_DECEL = -3.0  # m/s² - 提高舒適減速度上限 (原 -2.5)
EMERGENCY_DECEL_THRESHOLD = -3.5  # m/s² - 緊急煞車閾值
SAFETY_TIME_BUFFER = 0.2  # 秒 - 大幅降低緩衝時間，允許近距離介入 (原 2.0)
...
class SpeedCameraControl:
  ...
  def _is_safe_to_intervene(self, v_ego_ms, cam_limit_kph, distance):
    ...
    # 只有極度不合理的減速才拒絕 (例如 -6.0G，通常是 GPS 漂移導致距離瞬間變零)
    if required_decel < -6.0:
        self.rejection_count += 1
        return False, required_decel, "impossible_decel"
    
    # 如果需要緊急煞車，記錄警告但允許介入 (交給 OP 縱向控制去處理 Jerk)
    ...
    # 檢查時間緩衝
    time_available = distance / v_ego_ms if v_ego_ms > 1.0 else 99.0
    
    if time_available < SAFETY_TIME_BUFFER:
        # 如果雖然時間短，但減速度還算溫和 (例如近距離低速)，則允許介入
        if required_decel > -2.0:
             return True, required_decel, "short_time_safe"
  ...
  def get_statistics(self):
    return {"interventions": self.intervention_count, "rejections": self.rejection_count, "cameras_loaded": len(self.cameras)}
