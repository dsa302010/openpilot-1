...
# --- [設定] 安全參數 ---
SAFETY_OFFSET = 15.0       # 安全門檻: 車速高於限速 15km/h 則忽略
EMERGENCY_DECEL = -4.5     # 急煞門檻: 物理減速度低於此值則忽略
  ...
  def get_target_speed(self, v_ego_ms, v_cruise_ms, lat, lon, bearing_deg):
    ...
    for i, cam in enumerate(nearby_cams):
      ...
      if dist > limit_radius:
        ...
        # [關鍵] 使用動態計算的 current_hold_dist
        # 通過後距離 dist 變大，target 會從 cam_limit 慢慢升回 v_cruise
        # 0 ~ current_hold_dist: 維持 cam_limit
        # current_hold_dist ~ limit_radius: 線性上升至 v_cruise
